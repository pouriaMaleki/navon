...
fn probe_rx_line(rx_gpio: i32) {
    ...
    let combined: &str = match (pd_verdict, pu_verdict) {
        ...
        (LineState::Low, LineState::High) => {
            "FLOATING — no external connection on this pin. \
             Most likely cause: TX/RX swapped (module's TX should go to MCU RX = GPIO21, \
             not MCU TX = GPIO20), or the GPS RX wire is on a different physical pin, \
             or the module is unpowered (a powered NEO-6M idles its TX line at 3.3 V even \
             when not transmitting). Try swapping the two data wires first."
        }
    ...
    // If the configured pin is electrically dead, sweep a curated set
    // of plausibly-free GPIOs and report any that look externally
    // driven. The most common reason for FLOATING on this board is
    // that the silkscreen pin labelled "GPIO21" (or wherever the user
    // wired the GPS-TX) doesn't actually map to ESP32-P4 GPIO21 — the
    // Pi-style header on the Waveshare 3.4C uses Pi *layout* without
    // preserving Pi *numbering*. The scan below tells us where the
    // wire really lands so the operator can either rewire or update
    // the RX GPIO constant.
    if matches!(pd_verdict, LineState::Low) && matches!(pu_verdict, LineState::High) {
        scan_for_active_high_pins(rx_gpio);
    }
}
