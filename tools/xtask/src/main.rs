...
impl Workspace {
    fn new() -> Result<Self, String> {
        let root = Path::new(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .nth(2)
            .ok_or("xtask manifest should live inside the workspace (tools/xtask)".to_owned())?
            .to_path_buf();
        ...
        Ok(Self {
            emulator_web: root.join("device/emulator/web"),
            wasm_pkg: root.join("device/emulator/web/wasm-pkg"),
            wasm_crate: root.join("device/core/render-core-wasm"),
            map_data: root.join("data/map-data/city.svm"),
            device_map_data: root.join("data/map-data/city-small.svm"),
            xtask_tmp: root.join(".xtask/tmp"),
            firmware_crate: root.join("device/firmware"),
            partitions_csv: root.join("device/firmware/partitions.csv"),
...
mod tests {
    ...
    fn test_workspace() -> Workspace {
        Workspace {
            ...
            emulator_web: PathBuf::from("/work/device/emulator/web"),
            wasm_pkg: PathBuf::from("/work/device/emulator/web/wasm-pkg"),
            wasm_crate: PathBuf::from("/work/device/core/render-core-wasm"),
            map_data: PathBuf::from("/work/data/map-data/city.svm"),
            device_map_data: PathBuf::from("/work/data/map-data/city-small.svm"),
            xtask_tmp: PathBuf::from("/work/.xtask/tmp"),
            firmware_crate: PathBuf::from("/work/device/firmware"),
            partitions_csv: PathBuf::from("/work/device/firmware/partitions.csv"),
    ...
    fn builds_expected_wasm_pack_command() {
        ...
        assert_eq!(
            ...
            vec![
                OsString::from("build"),
                OsString::from("/work/device/core/render-core-wasm"),
                OsString::from("--target"),
                OsString::from("web"),
                OsString::from("--out-dir"),
                OsString::from("/work/device/emulator/web/wasm-pkg"),
                OsString::from("--out-name"),
                OsString::from("render_core_wasm"),
    ...
    fn builds_expected_npm_dev_command() {
        ...
        assert_eq!(spec.cwd, PathBuf::from("/work/device/emulator/web"));
    ...
    fn builds_expected_firmware_build_command() {
        ...
        assert_eq!(spec.cwd, PathBuf::from("/work/device/firmware"));
    }
    ...
    fn builds_expected_espflash_save_image_command() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                OsString::from(DEVICE_FLASH_SIZE),
                OsString::from("--partition-table"),
                OsString::from("/work/device/firmware/partitions.csv"),
