...
- OSM routing currently goes through the `project-osrm.org` demo server's `bike` profile. The demo's bike profile is generic and does not strongly prefer cycleway / path infrastructure over residential streets, so in cities with dense bike networks (e.g. Helsinki) routes can still pick driving roads. The pinned regression test in `routingSession.test.ts` documents this. The intended follow-up is to introduce a `BrouterRoutingAdapter` against `https://brouter.de/brouter` (free, public, OSM-based, cycling-specific profiles `trekking` / `safety` / `fastbike`) and route the OSM source through it. BRouter returns GeoJSON LineStrings rather than OSRM polylines, so a separate parser is needed; the existing `OsrmRoutingAdapter` shape (alternatives + maneuvers + summary) is the target normalization.
