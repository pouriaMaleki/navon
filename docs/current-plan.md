...
- iOS direct share imports now replace any existing preview/device route state before planning the imported destination or route, so a new Google Maps handoff becomes the current route instead of layering onto stale state.
