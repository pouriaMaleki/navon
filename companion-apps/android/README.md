...
- Android SDK packages required by CI are:
  ...
  - `build-tools;35.0.0`
- Keep provider credentials out of the repo.
  - HSL keys should be entered through the app's settings flow during local development.
  - Any future Android-only local overrides should live in untracked local configuration such as `local.properties`, not committed source files.
