...
export class DebuggerStore {
  ...
  generateTestStubs(): string {
    const export_ = this.exportAnnotations();
    if (!export_ || export_.annotations.length === 0) return "// No annotations to generate tests from.";

    const parts: string[] = [
      "// Generated test stubs from debugger annotations",
      `// Session: ${export_.sessionId}`,
      `// Exported: ${new Date(export_.exportedAtMs).toISOString()}`,
      "",
    ...
    for (const ann of export_.annotations) {
      if (ann.tag === "wrong_cue" || ann.tag === "missing_cue" || ann.tag === "wrong_timing") {
        imports.add("import { createTestGuidanceStore } from '../test/guidanceTestHarness.js';");
      }
      if (ann.tag === "gps_issue") {
        imports.add("import { projectOntoPolyline } from '../integrations/geo.js';");
      }
      if (ann.tag === "reroute_issue") {
        imports.add("import { createTestGuidanceStore } from '../test/guidanceTestHarness.js';");
      }
    }
    ...
    const sessionStart = this.session ? sessionStartTime(this.session.diagSession) : 0;

    for (const ann of export_.annotations) {
      const ctx = export_.eventContext[ann.id];
      const startSec = Math.round(ann.timeRangeMs[0] / 1000);
      const severityLabel = ann.severity === "bug" ? "BUG" : ann.severity === "improvement" ? "IMPROVEMENT" : "NOTE";
      const testName = `${severityLabel}: ${ann.tag} at ~${startSec}s — ${ann.note.slice(0, 80)}`;

      parts.push(`describe('${testName}', () => {`);

      const gpsEvents = [...(ctx?.before ?? []), ...(ctx?.during ?? [])]
        .filter((e) => e.data.kind === "locationUpdate");
      if (gpsEvents.length > 0) {
        parts.push("  const gpsTrace = [");
        for (const ge of gpsEvents.slice(0, 20)) {
          if (ge.data.kind === "locationUpdate") {
            parts.push(`    { lat: ${ge.data.lat}, lon: ${ge.data.lon} },`);
          }
        }
        parts.push("  ];");
        parts.push("");
      ...
      const cueEvents = ctx?.during?.filter((e) => e.data.kind === "audioCueDispatched") ?? [];
      if (cueEvents.length > 0) {
        parts.push("  // Actual cues from diagnostics:");
        for (const ce of cueEvents) {
          if (ce.data.kind === "audioCueDispatched") {
            parts.push(`  //   "${ce.data.messageText}" (${ce.data.cueType})`);
          }
        }
        parts.push("");
      }

      const turnEvents = ctx?.during?.filter((e) => e.data.kind === "nextTurnAlerted") ?? [];
      if (turnEvents.length > 0) {
        parts.push("  // Actual UI turn alerts from diagnostics:");
        for (const te of turnEvents.slice(0, 5)) {
          if (te.data.kind === "nextTurnAlerted") {
            parts.push(`  //   "${te.data.instructionText}" at ${te.data.distanceRemainingM}m`);
          }
        }
        parts.push("");
      }
      ...
      switch (ann.tag) {
        case "wrong_cue":
          parts.push("  test('should NOT dispatch this cue', () => {");
          parts.push("    // TODO: Set up the store with the GPS trace and route");
          parts.push("    // TODO: Advance to the point where the wrong cue fired");
          parts.push("    // TODO: Assert the cue was NOT dispatched");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "missing_cue":
          parts.push("  test('should dispatch the missing cue', () => {");
          parts.push("    // TODO: Set up the store with the GPS trace and route");
          parts.push("    // TODO: Advance to the point where the cue should have fired");
          parts.push("    // TODO: Assert the expected cue WAS dispatched");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "wrong_ui":
          parts.push("  test('should show correct next-turn UI', () => {");
          parts.push("    // TODO: Set up the store with the GPS trace and route");
          parts.push("    // TODO: Advance to the point where UI was wrong");
          parts.push("    // TODO: Assert the turn alert matches expected");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "missing_ui":
          parts.push("  test('should show next-turn UI', () => {");
          parts.push("    // TODO: Set up the store with the GPS trace and route");
          parts.push("    // TODO: Assert a turn alert IS present");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "wrong_timing":
          parts.push("  test('should fire cue at correct distance', () => {");
          parts.push("    // TODO: Set up the store with the GPS trace and route");
          parts.push("    // TODO: Assert cue fires at the expected distance threshold");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "gps_issue":
          parts.push("  test('should project GPS onto route correctly', () => {");
          parts.push("    // TODO: Set up route geometry");
          parts.push("    // TODO: Project each GPS point and verify it lands on the route");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        case "reroute_issue":
          parts.push("  test('should trigger reroute at correct threshold', () => {");
          parts.push("    // TODO: Set up the store, go off-route");
          parts.push("    // TODO: Assert reroute triggers at the correct distance/time");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
          break;
        default:
          parts.push("  test('TODO: define expected behavior', () => {");
          parts.push("    expect(true).toBe(true); // placeholder");
          parts.push("  });");
      }
