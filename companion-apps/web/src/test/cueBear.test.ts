...
describe("CueEngine — slight turns are first-class cues", () => {
  it("slightLeft fires nextTurnInAbout on route start when >50m away", () => {
    const t = tick(initialCueEngineState(), baseSnapshot({
      maneuvers: [M_SLIGHT_LEFT("m1", 200)], progressDistanceM: 0,
    }));
  ...
  it("slightRight fires turn50m approach when rider enters 50m window", () => {
    const route = baseSnapshot({ maneuvers: [M_SLIGHT_RIGHT("m2", 300)], routeTotalDistanceM: 500 });
  ...
  it("slightLeft back-to-back pair gets combined turn50m", () => {
    ...
    const t = tick(initialCueEngineState(), baseSnapshot({
      maneuvers: [M_SLIGHT_LEFT("m1", 40), M_RIGHT("m2", 60)], progressDistanceM: 0,
    }));
  ...
  it("slightLeft near route end substitutes arrivingInM", () => {
    const t = tick(initialCueEngineState(), baseSnapshot({
      maneuvers: [M_SLIGHT_LEFT("last", 480)], progressDistanceM: 0, routeTotalDistanceM: 500,
    }));
  ...
  it("slightRight turn10m is suppressed when arrived", () => {
    const t = tick(initialCueEngineState(), baseSnapshot({
      maneuvers: [M_SLIGHT_RIGHT("m1", 50)], progressDistanceM: 40, arrived: true,
    }));
