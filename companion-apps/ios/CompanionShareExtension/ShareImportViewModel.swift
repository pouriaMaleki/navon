...
final class ShareImportViewModel {
    ...
    private let makeDebugContext: () -> SharedImportDebugContext
    private let onSuccess: (SharedImportEnvelope) -> Void
    private let onError: (String) -> Void

    init(
        sharedStore: SharedImportStore,
        makeDebugContext: @escaping () -> SharedImportDebugContext,
        onSuccess: @escaping (SharedImportEnvelope) -> Void,
        onError: @escaping (String) -> Void
    ) {
        self.sharedStore = sharedStore
        self.makeDebugContext = makeDebugContext
        self.onSuccess = onSuccess
        self.onError = onError
    }

    func process(items: [NSExtensionItem]) async {
        let providers = items.flatMap { $0.attachments ?? [] }
        guard !providers.isEmpty else {
            onError("Nothing to import.")
            return
        }
        ...
        guard var selected = preferredEnvelope(from: candidates) else {
            onError("Could not read this shared item yet.")
            return
        }
        selected.debugTrail = buildSelectionDebugTrail(from: candidates, selectedID: selected.id)
        selected.debugContext = makeDebugContext()
    ...
    private func buildSelectionDebugTrail(from envelopes: [SharedImportEnvelope], selectedID: String) -> [String] {
        ...
        let ranked = envelopes.sorted { lhs, rhs in
            ...
            if lhsKey == rhsKey { return lhs.id < rhs.id }
