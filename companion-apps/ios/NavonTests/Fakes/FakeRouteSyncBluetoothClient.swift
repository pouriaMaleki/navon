...
final class FakeRouteSyncBluetoothClient: RouteSyncBluetoothClient {
    ...
    var scanResult: Result<String, Error> = .success("Navon")
    var connectResult: Result<String, Error> = .success("Navon")
    var connectToPairedResult: Result<String, Error> = .success("Navon")
    var connectToAdvertisedResult: Result<ConnectedPeripheralInfo, Error> = .success(
        ConnectedPeripheralInfo(name: "Navon", identifier: "B6C8AE6A-1A8C-4F2E-9F1A-9D2B0C3E4F5A")
    )
